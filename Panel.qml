...
Panel {
  ...
  readonly property var categories: [
    {key: "agents", title: "Agents", count: agents.length},
    {key: "spaces", title: "Spaces", count: spaces.length},
    {key: "tabs", title: "Tabs", count: tabs.length},
    {key: "shells", title: "Shells", count: shells.length}
  ]
  ...
  function fetch() {
    if (refreshing || itemMenu.visible) return
    refreshing = true
  ...
  function handleKey(event) {
    ...
    else if (control && event.key === Qt.Key_R) fetch()
  ...
  KeyboardPanel {
    ...
    contentHeight: panel.fittedContentHeight(chrome.implicitHeight + (root.rows.length ? Math.min(root.rowsHeight, Style.space(285)) : Style.space(70)) + footer.height + Style.space(12), Style.space(430))
    ...
    FocusScope {
      ...
      Column {
        ...
        Row {
          ...
          Repeater {
            ...
            delegate: Action {
              ...
              chosen: root.view === modelData.key || (root.view === "detail" && root.breadcrumbs.length > 0 && modelData.key === (root.breadcrumbs[0].kind === "tab" ? "tabs" : "spaces"))
              tabStyle: true
        ...
        Rectangle {
          ...
          border.width: 1; border.color: search.activeFocus ? root.alpha(root.foreground, 0.32) : root.line
          ...
          Controls.TextField {
            ...
            text: root.query
            placeholderText: root.view === "detail" ? "Filter this space…" : "Find " + root.view + "…"
          ...
          Action { anchors.right: parent.right; anchors.verticalCenter: parent.verticalCenter; visible: root.query !== ""; text: "×"; hint: "Clear search"; onClicked: { root.query = ""; search.forceActiveFocus() } }
        }
        ...
        delegate: Item {
          ...
          Controls.AbstractButton {
            ...
            background: Rectangle {
              ...
              color: rowButton.hovered || rowButton.activeFocus || root.selectedKey === listRow.modelData.key ? root.alpha(root.foreground, 0.09)
                : listRow.modelData.score >= 3 ? root.alpha(root.tone(listRow.modelData.score), 0.055) : "transparent"
              border.width: rowButton.activeFocus ? 1 : 0
              border.color: root.alpha(root.foreground, 0.35)
              Rectangle { visible: listRow.modelData.score >= 3; x: 0; y: Style.space(6); width: Style.space(2); height: parent.height - Style.space(12); radius: 1; color: root.tone(listRow.modelData.score) }
              Rectangle { x: Style.space(9); anchors.bottom: parent.bottom; width: parent.width - x * 2; height: 1; color: root.alpha(root.foreground, 0.045) }
            }
            contentItem: Item {
              Rectangle {
            ...
            Controls.Menu {
              ...
              Instantiator {
                ...
                delegate: Controls.MenuItem {
                  ...
                  background: Rectangle { color: sessionOption.highlighted ? root.alpha(root.foreground, 0.1) : "transparent"; radius: Style.space(3) }
  ...
  component ContextAction: Controls.MenuItem {
    ...
    background: Rectangle { color: option.enabled && (option.highlighted || option.hovered) ? root.alpha(root.foreground, 0.1) : "transparent"; radius: Style.space(3) }
  }
  ...
  component Action: Controls.AbstractButton {
    ...
    background: Rectangle {
      radius: action.tabStyle ? 0 : Style.space(4)
      color: !action.enabled ? "transparent" : action.down ? root.alpha(root.foreground, 0.18) : action.hovered || action.activeFocus ? root.alpha(root.foreground, 0.10) : action.chosen && !action.tabStyle ? root.alpha(root.foreground, 0.13) : "transparent"
      border.width: action.activeFocus ? 1 : 0; border.color: root.alpha(root.foreground, 0.4)
      Rectangle { visible: action.tabStyle; anchors.bottom: parent.bottom; width: parent.width; height: action.chosen ? 2 : 1; color: action.chosen ? root.foreground : root.line }
    }
    contentItem: Label {
      id: actionText
